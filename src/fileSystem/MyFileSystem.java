...
public class MyFileSystem implements FileSystem {
    ...
    private DirectBlock getDirectBlock(int fd, MODE mode) {
        ...
        //direct
        if(blockNum <= 9){
        	return direct(inode, mode, blockOff, blockNum);
        //single indirection
        }else if(blockNum > 9 && blockNum < 10+IndirectBlock.COUNT) {
        	return singleIndirect(inode, mode, blockOff, blockNum);
        //double indirection
        }else if(blockNum>=10+IndirectBlock.COUNT && blockNum < 10+IndirectBlock.COUNT+(IndirectBlock.COUNT*IndirectBlock.COUNT)){
        	return doubleIndirect(inode, mode, blockOff, blockNum);
        //triple indirection
        }else if(blockNum>=10+IndirectBlock.COUNT+(IndirectBlock.COUNT*IndirectBlock.COUNT) && blockNum < 10+IndirectBlock.COUNT+Math.pow(IndirectBlock.COUNT, 2)+Math.pow(IndirectBlock.COUNT, 3)){
        	return tripleIndirect(inode, mode, blockOff, blockNum);
        }
    ...
    private DirectBlock direct(Inode inode, MODE mode, int blockOff, int blockNum) {
    ...
    private DirectBlock singleIndirect(Inode inode, MODE mode, int blockOff, int blockNum) {
    	...
    	int pointer = blockNum-10;
    	return getDirectSingle(inode, indirect, mode, blockOff, blockNum, pointer);
	}

	private DirectBlock doubleIndirect(Inode inode, MODE mode, int blockOff, int blockNum) {
		IndirectBlock indirect=new IndirectBlock();
    	...
    	int pointer = blockNum-11;
        return getDirectDouble(inode, indirect, mode, blockOff, blockNum, pointer);
	}
    ...
    private DirectBlock getDirectSingle(Inode inode, IndirectBlock indirect, MODE mode, int blockOff, int blockNum, int pointer) {
    	...
    	if(fresh){
            if(mode == MODE.r)
                return DirectBlock.hole;
            else if((indirect.ptr[pointer] = freeMap.find()) == 0)
                return null;
            freeMap.save();
    	}
